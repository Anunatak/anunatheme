...
class Enqueue {
	...
	public function buildInclude( $attrs, $footer ) {
		...
		if ( !isset( $attrs['version'] ) ) $attrs['version'] = null;
		...
		if ( pathinfo( $attrs['src'], PATHINFO_EXTENSION ) === 'css' ) {
			wp_enqueue_style( $attrs['as'], $attrs['src'], $attrs['dep'], $attrs['version'] );
		}
		else {
			wp_enqueue_script( $attrs['as'], $attrs['src'], $attrs['dep'], $attrs['version'], $footer );

			if ( isset( $attrs['localize'] ) ) {
				foreach ( $attrs['localize'] as $handle => $localize ) {
					wp_localize_script( $attrs['as'], $handle, $localize );
				}
			}
		}
	}
