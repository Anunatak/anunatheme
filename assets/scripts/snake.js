...
function ResetButton(canvas, game) {
  ...
  this.draw = function(context) {
    ...
    // context.fillText(App.snake.reset, centerX, centerY);
  };
}
...
function Score() {
  ...
  this.draw = function(context) {
    ...
    context.fillStyle = 'rgba(0, 0, 0, 0.1)';
    context.fillText(App.snake.score + ': ' + this.score, 10, 10);
  };
}
...
function Snake(options) {
  ...
  this.headCollidesWithWall = function(canvas) {
    ...
    if(xCollides || yCollides) {
    	jQuery(canvas).trigger('snake.collide');
    }
    return xCollides || yCollides;
  };
  ...
  this.headCollidesWithSelf = function() {
    ...
    for (i = 1; i < this.points.length; i++) {
      if (head.x === this.points[i].x && head.y === this.points[i].y) {
      	jQuery(canvas).trigger('snake.collide');
      	jQuery(canvas).trigger('snake.collide.self');
        return true;
      }
    }
  ...
  this.draw = function(context) {
    context.fillStyle = options.snakeColor;
